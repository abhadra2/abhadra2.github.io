...
function onbodyload() {
  ...
  let dataset = d3.csv("./data/WDI_Infant_Mortality_Rate.csv", rowConverter, function (data) {
    const countries = d3.map(data, function (d) {
      if(d.Region == "")
        return (d.Country_Code)
      else
        return null
    }).keys().filter(function(v) { return v != "null" });
    ...
    if(scene_param_value == 1){
      drawLineChartForCountry('WLD', vizobject.CountryColorMap, vizobject.countryMap);
    }
    if(scene_param_value == 2){
      drawLineChartForCountry('AFE', vizobject.RegionColorMap, vizobject.regionMap);
      drawLineChartForCountry('AFW', vizobject.RegionColorMap, vizobject.regionMap);
      drawLineChartForCountry('EUU', vizobject.RegionColorMap, vizobject.regionMap);
      drawLineChartForCountry('NAC', vizobject.RegionColorMap, vizobject.regionMap);
      drawLineChartForCountry('SSF', vizobject.RegionColorMap, vizobject.regionMap);
      drawLineChartForCountry('LIC', vizobject.RegionColorMap, vizobject.regionMap);
      drawLineChartForCountry('MIC', vizobject.RegionColorMap, vizobject.regionMap);
      drawLineChartForCountry('HIC', vizobject.RegionColorMap, vizobject.regionMap);
    ...
    if(scene_param_value == 3){
      ...
      drawLineChartForCountry('LBR', vizobject.CountryColorMap, vizobject.countryMap);
      drawLineChartForCountry('CAF', vizobject.CountryColorMap, vizobject.countryMap);
      drawLineChartForCountry('SLE', vizobject.CountryColorMap, vizobject.countryMap);
      drawLineChartForCountry('SOM', vizobject.CountryColorMap, vizobject.countryMap);
      drawLineChartForCountry('IND', vizobject.CountryColorMap, vizobject.countryMap);
      drawLineChartForCountry('USA', vizobject.CountryColorMap, vizobject.countryMap);
      drawLineChartForCountry('GBR', vizobject.CountryColorMap, vizobject.countryMap);
      drawLineChartForCountry('BWA', vizobject.CountryColorMap, vizobject.countryMap);
...
function drawGraph(scene) {
  ...
  if(scene ==1){
    svg.append('text')
      ...
      .style('font-size', 20)
      .text('Infant Mortality Rate (World)');
  }else if(scene==2){
    svg.append('text')
      .attr('x', width / 2 + 100)
      .attr('y', 90)
      ...
      .style('font-family', 'Helvetica')
      .style('font-size', 20)
      .text('Compare [Infant Mortality Rate] By Groups');
  }else if(scene == 3){
    svg.append('text')
      .attr('x', width / 2 + 100)
      .attr('y', 90)
      ...
      .style('font-family', 'Helvetica')
      .style('font-size', 20)
      .text('Compare [Infant Mortality Rate] By Countries');
  }
  ...
  svg.append('text')
    ...
    .style('font-family', 'Helvetica')
    .style('font-size', 14)
    .text('Infant Mortality Rate (per 1000 live birth)');
...
function handleCheckBoxEvent(country, screen_id) {
  ...
  if (checkbox.checked) {
    if(screen_id == 2)
      drawLineChartForCountry(country.id.substring(5,country.id.length-1), vizobject.RegionColorMap, vizobject.regionMap);
    if(screen_id == 3)
      drawLineChartForCountry(country.id.substring(5,country.id.length-1), vizobject.CountryColorMap, vizobject.countryMap);
  } else {
...
const caption_box_html_1 = '</br></br><li>Infant Mortality Rate is steadily declining in the world.</li></br><li>More than 130% reduction since 1990.\n' +
  '            </li>\n' +
  '            </br>\n' +
  '            <li>\n' +
  '                The Average drop per year is 1.25.\n' +
  '            </li>\n' +
  '            </br>\n' +
  '            <li>\n' +
  '                Average drop per year = (Rate in 1990 - Rate in 2020) / 30\n' +
  '            </li>';
const caption_box_html_2 = /*'</br> Explore the trends for various groups by selecting in the filter box.<br>' +
    ...
    '</br></br><li> There is universal <b><em>decline</em></b> IMR mainly <em>after</em> 2000.' +
  '</br></br><li> For <b>African</b> group of countries, IMR is still high.' +
  '</br></br><li> The IMR of <b>African</b> countries in 2020 is still way higher compare to the same for <b>North America</b> and <b>European Union</b> countries in 1990.'+
    '</br></br><li> The IMR of <b>low income group</b> countries are much more <b>higher</b> than <b>middle</b> or <b>high income groups</b>.';
...
const caption_box_html_3 = '<br> Explore the trends for various countries by selecting in the filter box.</br>' +
    '<br> Current Selection shows :'+
  '</br></br><li> Most of the countries has <u><b>improved</b></u> performance over 30 years. ' +
  '</br></br><li> Mainly countries in <b>Africa</b> continent has still high IMR. ' +
    '</br></br><li> Countries in <b>Europe</b> and <b>North America</b> in general has very low IMR. ' +
    '</br></br><li> Developing countries like <b>India</b> is constantly improving IMR year by year. ' +
...
var footer_box_html_1 = '<p style="font-family: Helvetica font-size: 15px font-style: italic">Note: Hover on the data points to get more information in a tooltip.';
...
function addAnnotationRect(group, color) {
  group.append('rect')
    .attr("x", vizobject.graph.xScale(2014))
    .attr("y", vizobject.graph.yScale(160))
    .attr('width', 300)
    .attr('height', 200)
    .attr('rx', 8)
    .attr('ry', 8)
...
function addannotations(sceneid){
  ...
  //addAnnotationRect(group, "yellow")
  if(sceneid == 1){
    ...
    addAnnotationIndicator(group, vizobject.graph.xScale(2020)+100, vizobject.graph.yScale(27.4)+90, '5px', 'yellow')
    addLine(group, vizobject.graph.xScale(2020)+100, vizobject.graph.yScale(27.4)+100, vizobject.graph.xScale(2020)+100, vizobject.graph.yScale(27.4)+30)
    addLine(group, vizobject.graph.xScale(2014)+100, vizobject.graph.yScale(32.3)+40, vizobject.graph.xScale(2020)+100, vizobject.graph.yScale(27.4)+30)
    ...
    addAnnotationText(group, vizobject.graph.xScale(2020) -50, vizobject.graph.yScale(27.4)+130, "Reduced from 65 deaths to 27 deaths ")
    addAnnotationText(group, vizobject.graph.xScale(2020) -50, vizobject.graph.yScale(27.4)+150, "in 2020 - a 59 percent reduction")
    addLine(group, vizobject.graph.xScale(2020)+100, vizobject.graph.yScale(27.4)+90, vizobject.graph.xScale(2020)+100, vizobject.graph.yScale(27.4)+110)

  }else if(sceneid == 2){
    addAnnotationIndicator(group, vizobject.graph.xScale(2020)+100, vizobject.graph.yScale(60.8)+110, '5px', 'yellow')
    addLine(group, vizobject.graph.xScale(2020)+100, vizobject.graph.yScale(60.8)+110, vizobject.graph.xScale(2020)+120, vizobject.graph.yScale(60.8)+110)
    addLine(group, vizobject.graph.xScale(2020)+120, vizobject.graph.yScale(60.8)+110, vizobject.graph.xScale(2020)+120, vizobject.graph.yScale(60.8)+220)
    addLine(group, vizobject.graph.xScale(2020)+100, vizobject.graph.yScale(60.8)+220, vizobject.graph.xScale(2020)+120, vizobject.graph.yScale(60.8)+220)
    addAnnotationIndicator(group, vizobject.graph.xScale(2020)+100, vizobject.graph.yScale(60.8)+220, '5px', 'yellow')
    addLine(group, vizobject.graph.xScale(2020)+120, vizobject.graph.yScale(60.8)+150, vizobject.graph.xScale(2020)-30, vizobject.graph.yScale(60.8) - 60)
...
function drawLineChartForCountry(countryid, colourMap, dataMap) {
  ...
  if(parseInt(all)==1) {
    ...
    if (parseInt(scene_param) == 2) {
      for (i in vizobject.countryList) {
        drawLineChartForCountry(vizobject.countryList[i], vizobject.RegionColorMap, vizobject.regionMap);
      }
    }
    if (parseInt(scene_param) == 3) {
      for (i in vizobject.countryList) {
        drawLineChartForCountry(vizobject.countryList[i], vizobject.CountryColorMap, vizobject.countryMap);
      }
    }
  } else {
